...
if (isset($list) && count($list) > 0) {
    foreach ($list as $item) {
        ...
        $area['description'] =  Yii::$app->controller->truncate($item->description, 35, 400);;
        $area['class'] = 'item' . $item->id;
        $area['image'] = $item->getThumbPath(385, 200, 'w');
...
<div class="map-wrapper">
    <img src="<?= \yii\helpers\Url::to('@web/source/img/map/olympic.png') ?>" usemap="#Map" class="map">
    <map name="Map" id="Map">
    </map>
</div>
...
$script = <<< JS
			...
			$(document).ready(function(){
				for(i=0; i<areas.length; i++)
				{
		            ...
		            areas[i].coords=areas[i].coords.split('|')
					//create image map
					for(b=0; b<areas[i].coords.length; b++)
					{
						if(areas[i].path != undefined)
							$('#Map').append('<area position="'+areas[i].position+'" title="'+areas[i].title+'" class="'+areas[i].class+'" target="_blank" href="'+areas[i].path+'" shape="poly" coords="'+areas[i].coords[b]+'" />');
						else	
							$('#Map').append('<area position="'+areas[i].position+'" title="'+areas[i].title+'" href="javascript:" shape="poly" coords="'+areas[i].coords[b]+'" />');
					}
					...
					if(areas[i].path != undefined)
					{
						...
						if(areas[i].capacity != undefined && areas[i].capacity.length>0)
							html+='<i>Sygymlylygy: '+areas[i].capacity+'</i>';
						
						html+='<b>'+areas[i].title;

						if(areas[i].sports != undefined && areas[i].sports.length>0)
