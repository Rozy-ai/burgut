...
/**
 * EventToParticipantSearch represents the model behind the search form of `common\models\wrappers\EventToParticipantWrapper`.
 */
class EventToParticipantSearch extends EventToParticipantWrapper
{
    ...
    {
        return [
            [['id', 'event_id', 'participant_id', 'type', 'team_id', 'score_for', 'score_against', 'point', 'result_state', 'category_id'], 'integer'],
        ];
    }
    ...
    {
        $query = EventToParticipantWrapper::find();
    ...
    public function searchForLeague($params)
    {
        ...
        $query->andFilterWhere([
            ...
            self::tableName() . '.participant_id' => $this->participant_id,
        ...
        if (isset($this->competition_group_id)) {
            $query->andFilterWhere(['e.competition_group_id' => $this->competition_group_id]);
        }

        $query->andWhere(['e.type' => CompetitionPhaseWrapper::TYPE_LEAGUE]);

        $query->groupBy(['participant_id']);
        $query->select('
            participant_id, 
